...
const ChargeTableadmin = ({ user }) => {
  ...
  useEffect(() => {
    const teacherData = localStorage.getItem('currentProf');
    if (!teacherData) {
      navigate('/teachers');
      return;
    }
...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const url = `http://localhost:5000/api/teacher-charge/admin?teacher=${encodeURIComponent(teacherName)}&semester=${selectedSemester}`;
  ...
  fetchData();
}, [teacher, selectedSemester]); // Changé ici - remplacer [user, selectedSemester] par [teacher, selectedSemester] // Recharger quand user change
...
export default ChargeTableadmin;